...
The good thing about the compile time checks is that they are bound with the modifier class. For example when you accidentally used incorrect return type you get back:
...
- `disable_modifier` (which disables all hooks if they are already in the global table, does nothing otherwise)
...
Ok but how does one call the original from the modifier class? Another thing that's inherited is a typedef named `original` which you can use to call the original methods. The original methods have identical signature to your detours and are generated by the macro to properly call back the target function. For example say you want to hook a function named `foo` and call it from your detour. You can do it like so:
```cpp
void foo()
{
  // do your stuff
