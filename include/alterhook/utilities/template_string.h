...
namespace utils
{
  ...
  namespace helpers
  {
    ...
    struct make_substring<startpos, template_string<chars...>,
                          std::index_sequence<indexes...>>
    {
      static_assert(sizeof...(indexes) < sizeof...(chars) - startpos,
                    "substr_t: range specified is out of bounds");
      typedef template_string<
          template_string<chars...>::data[startpos + indexes]..., '\0'>
          type;
    };
    ...
    struct tstr_concater<tstr1, template_string<chars...>,
                         std::index_sequence<indexes...>>
    {
  ...
  using subtstr_t =
      typename helpers::make_substring<startpos, tstr,
  ...
  namespace helpers
  {
    template <template_string_type tstr, template_string_type cmptstr,
              bool fits>
    inline constexpr bool begins_with_impl =
        std::same_as<subtstr_t<tstr, 0, cmptstr::count>, cmptstr>;
    template <template_string_type tstr, template_string_type cmptstr>
    inline constexpr bool begins_with_impl<tstr, cmptstr, false> = false;
  } // namespace helpers

  template <typename tstr, typename cmptstr>
  concept tstr_begins_with = helpers::begins_with_impl < tstr,
          cmptstr, cmptstr::size<tstr::size>;
  template <typename tstr1, typename tstr2>
  using tstr_concat_t = typename helpers::tstr_concater<
      tstr1, tstr2, std::make_index_sequence<tstr1::count>>::type;
  ...
  template <typename tstr>
  using tstr_remove_terminator_t = typename helpers::cast_away_terminator<
      tstr, std::make_index_sequence<tstr::count>, tstr_terminated<tstr>>::type;
  template <typename tstr>
  using tstr_add_terminator_t =
      typename helpers::nullterminate<tstr, tstr_terminated<tstr>>::type;
  template <typename tstr>
  ...
  namespace helpers
  {
    ...
    struct cstr_wrapper
    {
      ...
      consteval cstr_wrapper(const char (&cstr)[N])
      {
        std::copy_n(cstr, N, data);
      }
    };
    ...
    template <cstr_wrapper str, auto result = make_template_string<str>(
