...
namespace alterhook::exceptions
{
  ...
  typedef byte_array<instruction_buffer_size> instruction_buffer_t;
  ...
  {
#if utils_arm
  #define __alterhook_arm_specific(...) __VA_ARGS__
#else
  #define __alterhook_arm_specific(...)
#endif

    // clang-format off
    /**
     * @class alterhook::exceptions::trampoline::unsupported_instruction_handling
     * @brief Thrown when no specific handling logic was implemented for an
     * instruction that depends on it.
     *
     * This may happen either during relocation from the target to the
     * trampoline function or from one trampoline function to the other. An
     * example is the process of copying a trampoline function that has raw
     * pc-relative loads (on aarch64) which may result in the new offsets being
     * too large to fit in the instruction's encoding (however it deals with it
     * just fine when relocating from a target function as the logic is
     * implemented on the init method).
     */
    utils_generate_exception(
        unsupported_instruction_handling, trampoline_exception,
        stdattr(ALTERHOOK_API),
        fields((byte_array<24>, instr,
                hidden)__alterhook_arm_specific(, (bool, thumb, hidden))),
        base_args((const std::byte*, target)),
        reason("Instruction depends on unimplemented handling logic"),
    ...
    // clang-format off
    /**
     * @class alterhook::exceptions::trampoline::post_relocation_processing_fail
     * @brief Thrown when an instruction that was supposed to be fixed after
     * relocation of all the required instructions from target was finished,
     * failed to be patched.
     *
     * Some instructions need post-relocation processing because they either
     * depend on information gathered after the said process is finished or they
     * have been moved from their original location on the trampoline (which
     * happens when an instruction in the middle is erased) and their relative
     * offset needs to be fixed. So for example branch instructions that
     * transfer execution to an area that was also relocated need to have their
     * offset set after all instructions that are in between (some of which
     * might be generated) are in their place on the trampoline function. If
     * fixing failed (e.g. when the relative offset is too large to fit in the
     * instruction) this is thrown.
     * @fn alterhook::exceptions::trampoline::post_relocation_processing_fail::get_instruction_relocation_address
     * @brief retrieves the address to which the instruction that failed to be
     * fixed was relocated.
     * @fn alterhook::exceptions::trampoline::post_relocation_processing_fail::get_instruction_expected_target_address
     * @brief retrieves the address that the instruction was expected to work
     * with (e.g. if this is a branch instruction then it's the branch
     * destination or if it's a relative load then it's the address from which
     * it reads)
     */
    // clang-format on
    utils_generate_exception(
        post_relocation_processing_fail, trampoline_exception,
        stdattr(ALTERHOOK_API),
        fields((instruction_buffer_t, instr,
                hidden)__alterhook_arm_specific(, (bool, thumb, hidden)),
               (const std::byte*, instruction_relocation_address),
               (const std::byte*, instruction_expected_target_address)),
        base_args((const std::byte*, target)),
        reason("An instruction that was copied to the trampoline failed to be "
               "fixed after relocation was finished"),
        extra(std::string info() const override;));
