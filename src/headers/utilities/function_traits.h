...
namespace utils
{
	namespace helpers
	{
		...
		#define __utils_clean_function(calling_convention, cv, ref, exception) \
			...
			struct clean_function_type_helper<R calling_convention (args...) cv ref exception> \
			{ \
				typedef R type(args...); \
			};
		...
		#define __utils_clean_member_function(calling_convention, cv, ref, exception) \
			...
			struct clean_function_type_helper<R (calling_convention cls::*)(args...) cv ref exception> \
			{ \
		...
		#define __utils_clean_stl_function(calling_convention, cv, ref, exception) \
			template <typename R, typename... args> \
			struct clean_function_type_helper<std::function<R calling_convention (args...) cv ref exception>> \
			{ \
				typedef R type(args...); \
			};
	...
	// and defines the clean function type which means that it's an as simple as possible function type (includes none of what was mentioned 
	// above and looks like "ret_type(args...)"). This is useful when we only care about its return and argument types and want to do checks
	// on them without having overloads breaking.
	template <typename T>
	struct clean_function_type : helpers::clean_function_type_helper<std::remove_pointer_t<T>> {};
	template <typename T>
	using clean_function_type_t = typename helpers::clean_function_type_helper<std::remove_pointer_t<T>>::type;
	...
	namespace helpers
	{
		...
		#define __utils_stl_fn_check(calling_convention, cv, ref, exception) \
			template <typename R, typename... args> \
			inline constexpr bool is_stl_function_impl<std::function<R calling_convention (args...) cv ref exception>> = true;
		...
		#define __utils_unwrap_stl_function(calling_convention, cv, ref, exception) \
			template <typename R, typename... args> \
			struct unwrap_stl_function_impl<std::function<R calling_convention (args...) cv ref exception>> \
			{ \
	...
	template <typename T>
	inline constexpr bool is_stl_function_v = helpers::is_stl_function_impl<remove_cvref_t<T>>;
	template <typename T>
	struct is_stl_function
	{
		static constexpr bool value = helpers::is_stl_function_impl<remove_cvref_t<T>>;
	};
	template <typename T>
	utils_concept stl_function_type = is_stl_function_v<T>;

	template <typename T>
	struct unwrap_stl_function : std::add_pointer<typename helpers::unwrap_stl_function_impl<remove_cvref_t<T>>::type> {};
	template <typename T>
	...
	namespace helpers
	{
		...
		#if utils_cpp20
		template <typename T>
		concept no_ambiguous_callable = requires { typename call_overload_t<T>; } && !stl_function_type<T>;
		#else
		template <typename T, typename = void>
		inline constexpr bool no_ambiguous_callable = false;
		template <typename T>
		inline constexpr bool no_ambiguous_callable<T, std::void_t<call_overload_t<T>>> = !stl_function_type<T>;
		#endif
		...
		{
		public:
			typedef function_traits_impl<clean_function_type_t<call_overload_t<T>>> call_type;
		...
		template <typename R, typename... args>
		struct function_traits_impl<helpers::thiscall_pfn_tag<R>(args...)> : function_traits_impl<R(args...)> {};
		#endif
	}
	...
	template <typename T>
	utils_concept function_type = std::is_function_v<clean_type_t<T>> || std::is_member_function_pointer_v<remove_cvref_t<T>> || is_stl_function_v<T>;
	template <typename T>
	utils_concept callable_type = function_type<T> || helpers::no_ambiguous_callable<remove_cvref_t<T>>;
	...
	{
		#if utils_cc_assertions
		static constexpr calling_convention calling_convention = helpers::get_calling_convention<helpers::call_overload_t<T>>();
		#endif
	};
	template <function_type T>
	struct function_traits<T> : helpers::function_traits_impl<clean_function_type_t<T>>
	{
		#if utils_cc_assertions
		static constexpr calling_convention calling_convention = helpers::get_calling_convention<T>();
		#endif
	};
	#else
	namespace helpers
	{
		...
		template <typename T>
		struct function_traits_sfinae<T, std::enable_if_t<no_ambiguous_callable<T>>> : function_traits_impl<T>
		{
			#if utils_cc_assertions
			static constexpr calling_convention calling_convention = get_calling_convention<call_overload_t<T>>();
			#endif
		};
		template <typename T>
		struct function_traits_sfinae<T, std::enable_if_t<function_type<T>>> : function_traits_impl<clean_function_type_t<T>>
		{
			#if utils_cc_assertions
			static constexpr calling_convention calling_convention = get_calling_convention<T>();
			#endif
		};
	}
	template <typename T>
	...
	#if utils_cc_assertions
	template <typename T>
	inline constexpr calling_convention fn_calling_convention_v = function_traits<T>::calling_convention;
	#endif
	template <typename T>
	using fn_class_t = std::remove_const_t<std::remove_pointer_t<fn_argument_t<T, 0>>>;

	#if utils_cc_assertions
	#if utils_cpp20
	template <typename T1, typename T2>
	concept compatible_calling_convention_with = requires { typename function_traits<T1>::return_type; typename function_traits<T2>::return_type; } &&
		(
			(fn_calling_convention_v<T1> == fn_calling_convention_v<T2>) ||
			(
				(fn_calling_convention_v<T1> == calling_convention::__THISCALL && fn_calling_convention_v<T2> == calling_convention::__FASTCALL) ||
				(fn_calling_convention_v<T1> == calling_convention::__FASTCALL && fn_calling_convention_v<T2> == calling_convention::__THISCALL)
				)
			);
	#else
	namespace helpers
	{
		...
		template <typename T1, typename T2>
		inline constexpr bool compatible_calling_convention_with_sfinae<T1, T2, std::void_t<fn_return_t<T1>, fn_return_t<T2>>> =
			(fn_calling_convention_v<T1> == fn_calling_convention_v<T2>) ||
			(
				(fn_calling_convention_v<T1> == calling_convention::__THISCALL && fn_calling_convention_v<T2> == calling_convention::__FASTCALL) ||
				(fn_calling_convention_v<T1> == calling_convention::__FASTCALL && fn_calling_convention_v<T2> == calling_convention::__THISCALL)
	...
	template <typename T1, typename T2>
	inline constexpr bool compatible_calling_convention_with = helpers::compatible_calling_convention_with_sfinae<T1, T2>;
	#endif
	#endif
	...
	namespace helpers
	{
		...
		inline constexpr bool all_arguments_same_check<std::index_sequence<indexes...>, fn_1, fn_2> = (std::is_same_v<fn_argument_t<fn_1, indexes>, fn_argument_t<fn_2, indexes - 1>> && ...);
		...
		#if utils_cpp20
		template <typename T1, typename T2>
		concept have_compatible_member_fn_first_args = requires
		{
		...
		template <typename T1, typename T2>
		utils_concept have_all_args_same = fn_arity_v<T1> == fn_arity_v<T2> && all_arguments_same_check_2<std::make_index_sequence<fn_arity_v<T1>>, T1, T2>;

		#if utils_cpp20
		template <typename T1, typename T2>
		concept have_all_fastcall_args_compatible_with_thiscall = have_compatible_member_fn_first_args<T1, T2>&& requires
		{
			typename fn_argument_t<T1, 1>;
		} && sizeof(fn_argument_t<T1, 1>) == sizeof(void*) && fn_arity_v<T2> == (fn_arity_v<T1> -1) && all_arguments_same_check<make_index_sequence_t<fn_arity_v<T1>, 2>, T1, T2>;
		#else
		template <typename T1, typename T2, typename = void>
		inline constexpr bool have_all_fastcall_args_compatible_with_thiscall = false;
		template <typename T1, typename T2>
		inline constexpr bool have_all_fastcall_args_compatible_with_thiscall<T1, T2, std::void_t<fn_argument_t<T1, 1>>> =
			have_compatible_member_fn_first_args<T1, T2> && sizeof(fn_argument_t<T1, 1>) == sizeof(void*) && fn_arity_v<T2> == (fn_arity_v<T1> -1) && all_arguments_same_check<make_index_sequence_t<fn_arity_v<T1>, 2>, T1, T2>;
		#endif

		template <typename T1, typename T2>
		utils_consteval bool have_compatible_fn_args()
		{
			// on x64 the calling convention is defaulted to 4-register parameter passing and we can't get the right results via template overloading.
			// so best we can do is verify that all args are the same (it is pretty much enough since there is no such need as an additional argument
			// to store 'edx' when trying to hook a __thiscall function with a __fastcall one on x64)
			#if utils_cc_assertions
			if constexpr (!compatible_calling_convention_with<T1, T2> || fn_calling_convention_v<T1> == fn_calling_convention_v<T2>)
			{
				return have_all_args_same<T1, T2>;
			}
			else
			{
				if constexpr (fn_calling_convention_v<T1> == calling_convention::__FASTCALL)
				{
					if constexpr (fn_arity_v<T2> == 1)
						return fn_arity_v<T1> == fn_arity_v<T2> && have_compatible_member_fn_first_args<T1, T2>;
					else
						return have_all_fastcall_args_compatible_with_thiscall<T1, T2>;
				}
				else if constexpr (fn_arity_v<T1> == 1 && fn_arity_v<T1> == fn_arity_v<T2>)
				{
	...
	utils_concept compatible_function_arguments_with = helpers::have_compatible_fn_args<fn_1, fn_2>();
	...
	namespace helpers
	{
		...
		#define __utils_convert_to_fn_pointer_impl(calling_convention, cv, ref, exception) \
			template <typename R, typename cls, typename... args> \
			struct convert_to_fn_pointer_impl<R (calling_convention cls::*)(args...) cv ref exception> \
			{ \
	...
	template <typename T>
	struct convert_to_fn_pointer<T, std::enable_if_t<member_function_type<T>>> : helpers::convert_to_fn_pointer_impl<T> {};
	#endif
	template <typename T>
	...
	// only non-capturing lambdas are objects that can be casted to function pointers (and maybe user types that define operator ret(*)(args...) but we
	...
	concept non_capturing_lambda = requires(std::remove_cvref_t<T> lmbd)
	{
		...
		{ static_cast<std::add_pointer_t<helpers::lambda_to_fn_t<helpers::call_overload_t<std::remove_cvref_t<T>>>>>(lmbd) };
	};
	#else
	namespace helpers
	{
		...
		template <typename T>
		inline constexpr bool non_capturing_lambda_sfinae_2<T,
			std::void_t<decltype(static_cast<std::add_pointer_t<helpers::lambda_to_fn_t<helpers::call_overload_t<remove_cvref_t<T>>>>>(std::declval<remove_cvref_t<T>>()))>> = true;

		template <typename T, typename = void>
		inline constexpr bool non_capturing_lambda_sfinae = false;
		template <typename T>
		inline constexpr bool non_capturing_lambda_sfinae<T, std::void_t<fn_return_t<remove_cvref_t<T>>>> =
			fn_object_v<remove_cvref_t<T>> && non_capturing_lambda_sfinae_2<T>;
	}
	template <typename T>
	inline constexpr bool non_capturing_lambda = helpers::non_capturing_lambda_sfinae<T>;
	#endif
	...
	namespace helpers
	{
		...
		template <typename... current_pairs, typename first, typename second, typename... rest>
		struct make_type_pairs_impl<type_sequence<current_pairs...>, first, second, rest...>
			: make_type_pairs_impl<type_sequence<current_pairs..., std::pair<first, second>>, rest...> {};
		...
		inline constexpr bool dtr_storage_pairs_helper = undefined_struct<seq>::value;
		template <typename first_elem, typename second_elem, typename... rest_pairs>
		inline constexpr bool dtr_storage_pairs_helper<type_sequence<std::pair<first_elem, second_elem>, rest_pairs...>> =
			(callable_type<first_elem> && function_type<remove_cvref_t<second_elem>> && std::is_lvalue_reference_v<second_elem>) && dtr_storage_pairs_helper<type_sequence<rest_pairs...>>;
		template <>
		inline constexpr bool dtr_storage_pairs_helper<type_sequence<>> = true;
	}

	template <typename... types>
	struct make_type_pairs : helpers::make_type_pairs_impl<type_sequence<>, types...>
	{
	...
	template <typename... types>
	utils_concept detour_and_storage_pairs = helpers::dtr_storage_pairs_helper<make_type_pairs_t<types...>>;
}
