...
#define __alterhook_for_hex_digit0(FN, PREFIX, SEPERATOR)                      \
  ...
  SEPERATOR()                                                                  \
  FN(PREFIX##1)                                                                \
  SEPERATOR() FN(PREFIX##2) SEPERATOR() FN(PREFIX##3) SEPERATOR()              \
      FN(PREFIX##4) SEPERATOR() FN(PREFIX##5) SEPERATOR() FN(PREFIX##6)        \
          SEPERATOR() FN(PREFIX##7) SEPERATOR() FN(PREFIX##8) SEPERATOR()      \
              FN(PREFIX##9) SEPERATOR() FN(PREFIX##A) SEPERATOR()              \
                  FN(PREFIX##B) SEPERATOR() FN(PREFIX##C) SEPERATOR()          \
                      FN(PREFIX##D) SEPERATOR() FN(PREFIX##E) SEPERATOR()      \
                          FN(PREFIX##F)

#define __alterhook_for_hex_digit(FN, SEPERATOR)                               \
  __alterhook_for_hex_digit0(                                                  \
      FN, 0x0, SEPERATOR) SEPERATOR() __alterhook_for_hex_digit0(FN, 0x1,      \
                                                                 SEPERATOR)    \
      SEPERATOR() __alterhook_for_hex_digit0(FN, 0x2, SEPERATOR) SEPERATOR()   \
          __alterhook_for_hex_digit0(                                          \
              FN, 0x3,                                                         \
              SEPERATOR) SEPERATOR() __alterhook_for_hex_digit0(FN, 0x4,       \
                                                                SEPERATOR)     \
              SEPERATOR() __alterhook_for_hex_digit0(                          \
                  FN, 0x5,                                                     \
                  SEPERATOR) SEPERATOR() __alterhook_for_hex_digit0(FN, 0x6,   \
                                                                    SEPERATOR) \
                  SEPERATOR() __alterhook_for_hex_digit0(FN, 0x7, SEPERATOR)   \
                      SEPERATOR() __alterhook_for_hex_digit0(                  \
                          FN, 0x8, SEPERATOR) SEPERATOR()                      \
                          __alterhook_for_hex_digit0(FN, 0x9,                  \
                                                     SEPERATOR) SEPERATOR()    \
                              __alterhook_for_hex_digit0(FN, 0xA, SEPERATOR)   \
                                  SEPERATOR() __alterhook_for_hex_digit0(      \
...
#define __alterhook_virtual_function_array_set()                               \
  ...
  //{
  // if (asm_instruction(*reinterpret_cast<BYTE*>(address)) ==
  // asm_instruction::JMP) address += sizeof(JMP) +
  // *reinterpret_cast<intptr_t*>(address + 1);
  // return address;
//}
#endif
...
// TO FIX
#if utils_windows
// if (*reinterpret_cast<BYTE*>(address) == 0xFF &&
// *reinterpret_cast<BYTE*>(address + 1) == 0x25)
//{
        ...
        do
        {
                ...
                case asm_instruction::MOV:
                        if (thisregister != asm_register::UNKNOWN)
                                return false;
                        // check if it loads from ecx (where the this pointer is
stored) if (asm_register(hs.modrm_rm) != asm_register::ECX) return false;
                        thisregister = asm_register(hs.modrm_reg);
                        break;
                case asm_instruction::JMP_ABS:
                        // check if it uses the register specified in mov (if it
doesn't then it's not a thunk) if (hs.modrm_reg != 4 ||
asm_register(hs.modrm_rm) != thisregister || hs.modrm_mod == 3) return false;
                        return true;
                case asm_instruction::JMP_SHORT:
                        instruction_pointer += sizeof(JMP_SHORT) +
static_cast<INT8>(hs.imm.imm8); continue; case asm_instruction::JMP:
                        instruction_pointer += sizeof(JMP) +
static_cast<int>(hs.imm.imm32); continue; default: return false;
                }
