...
namespace alterhook
{
	...
	void memory_block::init()
	{
		...
		do
		{
			...
			free = pfree;
			++pfree;
		} while (reinterpret_cast<uintptr_t>(pfree) - reinterpret_cast<uintptr_t>(this) <= memory_block_size - memory_slot_size);
	...
	{
		utils_assert(is_valid_slot(pslot), "memory_block::free_slot: Memory slot passed is invalid");
		pslot->next = free;
		free = pslot;
		--used_count;
	}
	...
	#if utils_windows64
	static std::byte* find_prev_free_region(std::byte* address, std::byte* minaddr, DWORD ag)
	{
		MEMORY_BASIC_INFORMATION mbi;
		for (
			uintptr_t regaddr = utils_align(reinterpret_cast<uintptr_t>(address), ag) - ag;
			regaddr >= reinterpret_cast<uintptr_t>(minaddr) && VirtualQuery(reinterpret_cast<void*>(regaddr), &mbi, sizeof(mbi));
	...
	static std::byte* find_next_free_region(std::byte* address, std::byte* maxaddr, DWORD ag)
	{
		MEMORY_BASIC_INFORMATION mbi;
		for (
			uintptr_t regaddr = utils_align(reinterpret_cast<uintptr_t>(address), ag) + ag;
			regaddr <= reinterpret_cast<uintptr_t>(maxaddr) && VirtualQuery(reinterpret_cast<void*>(regaddr), &mbi, sizeof(mbi));
			regaddr = utils_align(reinterpret_cast<uintptr_t>(mbi.AllocationBase) + mbi.RegionSize + (ag - 1), ag)
		)
		{
			if (mbi.State == MEM_FREE)
				return reinterpret_cast<std::byte*>(regaddr);
	...
	static std::tuple<std::byte*, std::byte*, DWORD> get_min_max_addr()
	{
		...
		GetSystemInfo(&info);
		return {
			static_cast<std::byte*>(info.lpMinimumApplicationAddress),
	...
	#else
		#define __alterhook_return_pslot return reinterpret_cast<std::byte*>(pblock->get_slot())
	#endif
	...
	std::byte* trampoline_buffer::allocate(__alterhook_alloc_arg)
	{
		...
		if (reinterpret_cast<uintptr_t>(origin) > max_memory_range && minaddr < origin - max_memory_range)
			minaddr = origin - max_memory_range;
		if (maxaddr > origin + max_memory_range)
			maxaddr = origin + max_memory_range;
		#define __alterhook_range_check && reinterpret_cast<std::byte*>(pblock) > minaddr && reinterpret_cast<std::byte*>(pblock) < maxaddr
		#define __alterhook_alloc_mem_block() \
			static_cast<memory_block*>(VirtualAlloc(reg, memory_block_size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE))
		#define __alterhook_raise_alloc_exception() \
			throw(exceptions::virtual_alloc_exception(errno, reg, memory_block_size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE))
		#else
			...
			#define __alterhook_raise_alloc_exception() \
				...
				#define __alterhook_raise_alloc_exception() \
					throw(exceptions::virtual_alloc_exception( \
						errno, nullptr, memory_block_size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE))
			#endif
		#endif
		
		memory_block* pblock = nullptr;
		for (memory_block* pblocktmp = buffer; pblock __alterhook_range_check; pblock = pblock->next)
		{
			if (pblocktmp->free)
			{
				pblock = pblocktmp;
		...
		std::byte* reg = origin;
		while (
			reg >= minaddr && (reg = find_prev_free_region(reg, minaddr, ag)) &&
		...
		if (!pblock)
		{
			reg = origin;
			while (
				reg <= maxaddr && (reg = find_next_free_region(reg, minaddr, ag)) &&
				!(pblock = __alterhook_alloc_mem_block())
			);
		}
		#else
	...
	void trampoline_buffer::deallocate(void* src) noexcept
	{
		...
		for (
			memory_block *target = reinterpret_cast<memory_block*>(utils_align(reinterpret_cast<uintptr_t>(src), memory_block_size)),
			*prev = nullptr, *pblock = buffer; pblock; prev = pblock, pblock = pblock->next
		)
		{
			if (pblock == target)
			{
				auto pslot = static_cast<memory_block::memory_slot*>(src);
				utils_assert(pblock->is_valid_slot(pslot), "trampoline_buffer::deallocate: Memory slot passed is invalid");
				#ifndef NDEBUG
